...
const CosmosAPI = ({ name, type }) => {
	...
	const PEERS = peerID ? `${peerID}@${name}-${type}-peer.cryptonode.id:${peerPort}${livePeers}` : ''
	const LIVE_PEERS = peerID ? `"${PEERS}"` : `"${livePeers.slice(1)}"`
	const SEEDS = seedID ? `${seedID}@${name}-${type}-seed.cryptonode.id:${seedPort}` : ''
	const gRPC = `${name}-${type}-grpc.cryptonode.id:${peerPort ? peerPort.slice(0, 2) : ''}090`
	...
	return (
		<AnimatedSection>
			<Head>
				...
				<meta name='description' content='Crypto Node ID 🚀 | Crypto Multipurpose Project' />
			</Head>
			...
			>
				...
				<div className='flex flex-col flex-wrap gap-1 mb-1'>
					<div className='flex flex-wrap gap-1 items-center'>
						<span>RPC: </span>
						<a href={`https://${name}-${type}-rpc.cryptonode.id`} target='_blank' rel='noopener referrer'>
							{`https://${name}-${type}-rpc.cryptonode.id`}
						</a>
						<Paragraph
							copyable={{
								text: `https://${name}-${type}-rpc.cryptonode.id`,
								tooltips: false
							}}
						/>
					</div>
					{name === 'namada' ? (
						...
						<>
							<div className='flex flex-wrap gap-1 items-center'>
								API:
								<a
									href={`https://${name}-${type}-api.cryptonode.id`}
									target='_blank'
									rel='noopener referrer'
								>
									{`https://${name}-${type}-api.cryptonode.id`}
								</a>
								<Paragraph
									copyable={{
										text: `https://${name}-${type}-api.cryptonode.id`,
										tooltips: false
									}}
								/>
							</div>
				...
				<h3 id='peer'>peers:</h3>
				<CodeSnippet theme={theme} code={`${peerID}@${name}-${type}-peer.cryptonode.id:${peerPort}`} />
				{SEEDS == '' ? (
					''
				) : (
					<div>
...
sed -i 's|^persistent_peers *=.*|persistent_peers = "'$PEERS'"|' $HOME/${path}/config/config.toml`}
				...
				<CodeSnippet
					theme={theme}
					code={`wget -O $HOME/${path}/config/addrbook.json https://${type}-files.cryptonode.id/${name}/addrbook.json`}
				/>
...
curl https://${type}-files.cryptonode.id/${name}/snap_${name}.tar.lz4 | lz4 -dc - | tar -xf - -C $HOME/${path}
...
peers="${peerID}@${name}-${type}-peer.cryptonode.id:${peerPort}${livePeers}"  
SNAP_RPC="https://${name}-${type}-rpc.cryptonode.id:443"
...
${
	!wasmPath
		? `
curl https://${type}-files.cryptonode.id/${name}/wasmPath_${name}.tar.lz4 | lz4 -dc - | tar -xf - -C $HOME/${wasmPath}`
		: ``
}
mv $HOME/${path}/priv_validator_state.json.backup $HOME/${path}/data/priv_validator_state.json
...
sudo systemctl restart ${bin} && sudo journalctl -u ${bin} -f`}
				...
				{wasmPath !== '' && wasmPath !== 'false' ? (
					<>
						...
						<CodeSnippet
							theme={theme}
							code={`curl https://${type}-files.cryptonode.id/${name}/wasmPath_${name}.tar.lz4 | lz4 -dc - | tar -xf - -C $HOME/${wasmPath} 
sudo systemctl restart ${bin} && sudo journalctl -u ${bin} -f`}
						/>
					</>
				) : (
